...
int cercarEntrada(const char *cami_parcial, unsigned int *p_inode_dir, unsigned int *p_inode, unsigned int *p_entrada, unsigned int reservar, int mode)
{
    ...
    if (i < num_ent) { // trobat
       ...
       if (reservar) {
            ...
            int r = reservarInode(tipus_inode, mode);
            if (r == -1) {
            ...
            if (mi_write_f(*p_inode_dir, &entra, num_ent * sizeof(entrada), sizeof(entrada)) == -1) { // escrivim els canvis
                printf("[directorios.c] ERROR: No s'ha pogut escriure l'entrada!\n");
                return -1;
            }

            *p_inode = r; // l'inode reservat es el seu inode (p_inode)
            *p_entrada = (estat.tamany / sizeof(entrada)); // numero de l'entrada (sa darrera)
    ...
    if (cercarEntrada(cami, p_inode_dir, p_inode, p_entrada, 0, mode) == -1) { // si no hi es...
        ...
        *p_entrada = 0;
        if (cercarEntrada(cami, p_inode_dir, p_inode, p_entrada, 1, mode) == -1) { // ...la crea
            printf("[directorios.c] ERROR: No s'ha pogut crear l'entrada.\n");
            alliberar(p_inode_dir, p_inode, p_entrada);
            sem_signal();
            return 0;
        }
        printf("[directorios.c] INFO: Cami '%s' creat\n", cami);
    } else {
        printf("[directorios.c] ERROR: La ruta ja existeix!\n"); // ja existeix
    }
    ...
    *p_entrada_enllac = 0;
    ...
    if (cercarEntrada(cami1, p_inode_dir, p_inode, p_entrada, 0, 7) == -1) { // entrada que se crea i enllaça. Comprova que no existeixi.
        ...
        *p_entrada = 0;
        ...
        if (cercarEntrada(cami1, p_inode_dir, p_inode, p_entrada, 1, 7) == -1) { // la cream
            printf("[directorios.c] ERROR: No s'ha pogut crear l'entrada nova.\n");
            alliberar(p_inode_dir_enllac, p_inode_enllac, p_entrada_enllac);
            alliberar(p_inode_dir, p_inode, p_entrada);
            sem_signal();
            return -1;
        ...
        if (mi_truncar_f(*p_inode_dir, estat.tamany - sizeof(entrada)) == -1) {
            printf("[directorios.c] ERROR: No s'ha pogut truncar\n");
            alliberar(p_inode_dir_enllac, p_inode_enllac, p_entrada_enllac);
            alliberar(p_inode_dir, p_inode, p_entrada);
            sem_signal();
            return -1;
        ...
        if (mi_write_f(*p_inode_dir, &ent, estat.tamany - sizeof(entrada), sizeof(entrada)) == -1) {
        ...
        in.links_directoris++;
        if (escriureInode(*p_inode_enllac, in) == -1) {
            printf("[directorios.c] ERROR: No s'ha pogut escriure l'inode\n");
            alliberar(p_inode_dir_enllac, p_inode_enllac, p_entrada_enllac);
            alliberar(p_inode_dir, p_inode, p_entrada);
            sem_signal();
            return -1;
...
int mi_chmod(const char *cami, unsigned char mode)
{
    ...
    if (mi_chmod_f(p_inode, mode) == -1) { // camviam els permisos a l'inode
        printf("[directorios.c] ERROR: No s'han canviat els permissos!\n");
        sem_signal();
        return -1;
    }
